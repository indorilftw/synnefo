...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        orm.ProjectMembership.objects.filter(state__in=ACCEPTED_STATES).\
            update(initialized=True)
        ms = orm.ProjectMembershipLog.objects.\
            filter(to_state__in=ACCEPTED_STATES).\
            values_list("membership", flat=True)
        orm.ProjectMembership.objects.filter(id__in=ms).\
            update(initialized=True)
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 1, 28, 12, 4, 23, 808946)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 1, 28, 12, 4, 23, 808896)'}),
