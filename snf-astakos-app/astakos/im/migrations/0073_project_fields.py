...
DISMISSED = 4
CANCELLED = 5
DELETED_STATES = [DISMISSED, CANCELLED]
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        projects = orm.Project.objects.select_related("application").all()
        for project in projects:
            app = project.application
            project.owner = app.owner
            project.realname = app.name
            project.homepage = app.homepage
            project.description = app.description
            project.end_date = app.end_date
            project.member_join_policy = app.member_join_policy
            project.member_leave_policy = app.member_leave_policy
            project.limit_on_members_number = app.limit_on_members_number
            project.private = app.private
            project.last_application = \
                project.chained_apps.all().order_by("-id")[0]
            project.save()

        orm.Project.objects.filter(state=NORMAL, application__state=PENDING).\
            update(state=UNINITIALIZED)
        orm.Project.objects.filter(
            state=NORMAL, application__state__in=DELETED_STATES).\
            update(state=DELETED)

        grants = orm.ProjectResourceGrant.objects.\
            filter(project_application__project__isnull=False).\
            select_related("project_application__project")
        ...
        initialized = orm.Project.objects.filter(state__in=INITIALIZED_STATES)
        objs = orm.ProjectResourceQuota.objects
        grants = objs.select_related("project").filter(project__in=initialized)
        grants = _partition_by(lambda g: g.project_id, grants)
        resources = orm.Resource.objects.all()
        new_grants = []
        for project in initialized:
            p_grants = grants.get(project.id, [])
            p_res = set(g.resource_id for g in p_grants)
            for resource in resources:
                if resource.id not in p_res:
                    new_grants.append(
                        orm.ProjectResourceQuota(
                            resource=resource,
                            project=project,
                            project_capacity=resource.project_default,
                            member_capacity=resource.project_default))

        orm.ProjectResourceQuota.objects.bulk_create(new_grants)
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 12, 12, 11, 1, 52, 856457)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 12, 12, 11, 1, 52, 856406)'}),
