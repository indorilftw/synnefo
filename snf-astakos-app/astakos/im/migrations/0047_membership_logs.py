...
ASSUMED_PRED = {
    REQUESTED:       None,
    ACCEPTED:        REQUESTED,
    LEAVE_REQUESTED: ACCEPTED,
    USER_SUSPENDED:  ACCEPTED,
    REJECTED:        REQUESTED,
    CANCELLED:       REQUESTED,
    REMOVED:         ACCEPTED,
}
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # First make logs for existing memberships
        logs = []
        ...
        membs = {}
        for m in memberships:
            membs[(m.project_id, m.person_id)] = m

        # Reconstruct memberships from history
        new_membs = {}
        ...
        # Make logs from history
        logs = []
        for mh in orm.ProjectMembershipHistory.objects.all():
            m = membs[(mh.project, mh.person)]
