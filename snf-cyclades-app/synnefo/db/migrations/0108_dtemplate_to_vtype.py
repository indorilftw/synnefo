...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for disk_template in orm.Flavor.objects\
                                .values_list("disk_template", flat=True)\
                                .distinct():
            vtype = orm.VolumeType.objects.create(name=disk_template,
                                                  disk_template=disk_template)
            orm.Flavor.objects.filter(disk_template=disk_template)\
                              .update(volume_type=vtype)
            orm.Volume.objects.filter(disk_template=disk_template)\
                              .update(volume_type=vtype)

    def backwards(self, orm):
        for volume_t in orm.VolumeType.objects.all():
            orm.Flavor.objects.update(disk_template=volume_t.disk_template)
            orm.Volume.objects.update(disk_template=volume_t.disk_template)
