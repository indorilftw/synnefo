...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for user in orm.AstakosUser.objects.all():
            user_handled = False
            # not moderated
            if not user.is_active and not user.activation_sent and not \
                    user.email_verified:
                user_handled = True
                user.moderated = False
                user.verification_code = str(uuid.uuid4())

            # moderated/not verified
            if not user.is_active and user.activation_sent and not \
                    user.email_verified:
                user_handled = True
                user.moderated = True
            ...
            # moderated/active/verified
            if user.is_active and user.email_verified:
                user_handled = True
                user.moderated = True
                user.moderated_at = user.auth_token_created
                user.accepted_policy = 'migration'
                user.accepted_email = user.email
                user.verified_at = user.moderated_at

            # verified
            if not user.is_active and user.email_verified and \
                ...
                user.deactivated_at = user.updated

            if not user_handled:
                print "ERROR: %s:%s is not migrated" % (user.pk, user.username)
