...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        objs = orm.AstakosUser.objects.filter(
            email_verified=False, moderated=True)

        inactive = list(objs.filter(is_active=False))
        if inactive:
            print "Reverted moderation of %s inactive users:" % len(inactive)
            print ", ".join([u.username for u in inactive])

            objs.filter(is_active=False).\
                update(moderated=False, is_rejected=False,
                       accepted_policy=None, moderated_at=None)

        active = objs.filter(is_active=True).count()
        if active:
            print ("***WARNING*** Found %s active users whose email has not "
                   "been verified. You may need to examine these cases." %
                   active)
