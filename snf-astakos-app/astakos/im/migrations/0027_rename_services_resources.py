...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for resource in resources:
            ...
            prefix = resource.service.name + "."
            if resource_name.startswith(prefix):
                print ("%s is already prefixed with service name."
                       % resource_name)
                continue
    ...
    def backwards(self, orm):
        ...
        for resource in resources:
            resource_name = resource.name
            prefix, sep, name = resource_name.partition('.')
            if prefix != resource.service.name:
                print ("Prefix %s does not equal the service name."
                       % prefix)
                continue
            resource.name = name
            resource.save()
