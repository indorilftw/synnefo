...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        objs = orm.ProjectResourceGrant.objects
        grants = objs.select_related("project_application").all()
        new_grants = []
        for grant in grants:
            application = grant.project_application
            max_members = application.limit_on_members_number
            if max_members is None:
                if application.state != APPROVED:
                    max_members = 1
                else:
                    objs = orm.ProjectMembership.objects
                    members = objs.filter(project=application.chain,
        ...
        orm.ProjectResourceGrant.objects.all().delete()
        orm.ProjectResourceGrant.objects.bulk_create(new_grants)
