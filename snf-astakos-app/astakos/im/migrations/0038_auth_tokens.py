...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        objs = orm.Component.objects.values('auth_token')
        dups = objs.annotate(count=models.Count('id')).filter(count__gt=1)
        dup_tokens = [item['auth_token'] for item in dups
                      if item['auth_token'] is not None]

        if dup_tokens:
            print "Found duplicate component auth tokens:"
            print dup_tokens
            orm.Component.objects.filter(auth_token__in=dup_tokens).update(
                auth_token=None)

        objs = orm.AstakosUser.objects.values('auth_token')
        dups = objs.annotate(count=models.Count('id')).filter(count__gt=1)
        dup_tokens = [item['auth_token'] for item in dups
                      if item['auth_token'] is not None]

        if dup_tokens:
            print "Found duplicate user auth tokens:"
            print dup_tokens
            orm.AstakosUser.objects.filter(auth_token__in=dup_tokens).update(
                auth_token=None)
